...
| | 이 저장소 |
|---|---|
| **웹앱** (이 폴더) | 설치 0 · 업로드 0 · 드래그&드롭 · `.hwp`·`.hwpx`·**`.docx`** · 수식(KaTeX) · 표·그림·글상자·도형 · 다중 파일 일괄 · **오프라인/설치형(PWA)** · 다크모드 |
| **CLI** (`cli/hwp2pdf.py`) | LibreOffice + **H2Orestart** 기반 *최고 충실도* 변환 (픽셀 단위로 똑같이 필요할 때) |

**출력 형식**: PDF(인쇄·벡터) · 이미지 PDF · 페이지별 PNG(여러 장은 ZIP) · 텍스트/마크다운(표·`$LaTeX$` 포함) 복사·다운로드 · 여러 문서 **전체 PDF를 ZIP**으로. 수식을 클릭하면 **LaTeX가 복사**됩니다.
...
- **`.hwp`** (구형 바이너리, OLE) : **자체 바이너리 파서**로 본문을 직접 해석 — 문단·글자서식·표·그림까지 **전체 내용**을 렌더(`cfb`로 컨테이너 읽기 + `pako`로 raw-inflate + HWP 레코드 파싱). 큰 사진은 자동 축소해 100MB+ 파일도 처리. (드물게 파싱 실패 시 첫 페이지 미리보기로 폴백) · 확장자가 `.hwpx`여도 실제 내용이 구형 `.hwp`면 **자동 감지**.
- **PDF 출력 2가지**
...
의존 라이브러리(CDN): JSZip(.hwpx 압축 해제), KaTeX(수식), jsPDF·html2canvas(이미지 PDF), `cfb`+`pako`(.hwp 바이너리 읽기·압축해제), docx-preview(.docx), Noto Sans/Serif KR(글꼴). `hwpbin.js`가 구형 .hwp 레코드를 파싱합니다.

## ⚠️ 알려진 한계
- `.hwp`(구형)는 자체 파서로 본문 전체(문단·표·그림)를 렌더하지만, 복잡한 도형/수식/정밀 레이아웃은 100% 동일하진 않을 수 있음 — 픽셀 단위가 필요하면 CLI(LibreOffice) 사용.
